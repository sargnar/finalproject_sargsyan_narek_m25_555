...
# valutatrade_hub/parser_service/updater.py (упростим логирование)
class RatesUpdater:
    ...
    def run_update(self, sources: List[str] = None) -> Dict[str, Any]:
        ...
        for source_name in sources:
            ...
            try:
                ...
                if rates and len(rates) > 1:
                    ...
                    print(f"Successfully fetched {len(rates)} rates from {source_name} in {fetch_time:.2f}s")
        ...
        if results['failed_sources'] and results['successful_sources']:
            print(f"Update completed with partial success")
        elif results['successful_sources']:
            print(f"Update completed successfully")
        else:
            print(f"Update failed for all sources")
